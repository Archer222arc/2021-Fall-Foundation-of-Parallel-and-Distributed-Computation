...
int main() {
  ...
  cudaMalloc((double**)&d_u1, N2 * N2 * (N/2+2) * size_of_double);
  cudaMalloc((double**)&d_u2, N2 * N2 * (N/2+2) * size_of_double);
  cudaMalloc((double**)&d_u, size_u);
  cudaMalloc((double**)&d_b, size_b);
  cudaMalloc((double**)&d_b1, size_b/2);
  cudaMalloc((double**)&d_b2, size_b/2);
  ...
  cudaMemset(d_u1, 0, N2 * N2 * (N/2+2) * size_of_double);
  cudaMemset(d_u2, 0, N2 * N2 * (N/2+2) * size_of_double);
  ...
  initialize<<<grid_dim, block_dim, blksize*size_of_double>>>(d_u1, d_b1, d_u, d_b, 1);
  initialize<<<grid_dim, block_dim, blksize*size_of_double>>>(d_u2, d_b2, d_u, d_b, 0);
  ...
  printf("time for initialization %g\n",elapsed_time/1000.0);
  ...
  for (k = 0; k < MAXITER; k++)
  {
      ...
      if (k == 0){
        ...
        initial_step<<<grid_dim,block_dim, blksize*size_of_double>>>(d_u2, d_b2, res);
        rb_kernel1<<<grid_dim, block_dim>>>(d_u1, d_u2, d_b1, res, 1);
      ...
      if (k > 0){
        ...
        //主要更新
        rb_kernel<<<grid_dim, block_dim, blksize*size_of_double>>>(d_u2, d_u1, d_b2, res, 0);
        rb_kernel1<<<grid_dim, block_dim>>>(d_u1, d_u2, d_b1, res, 1);
        cudaEventRecord(stop, 0);
        cudaEventSynchronize(stop);
        cudaEventElapsedTime(&time_k, start, stop);
      ...
      if (normr < RTOL * normr0 && k >= 33)
      {
        ...
        cudaEventCreate(&stop);
        cudaEventRecord(start, 0);
        residual<<<grid_dim, block_dim, blksize*size_of_double>>>(d_u2, d_u1, d_b2, res, 0);
  ...
  finalize<<<grid_dim,block_dim, blksize*size_of_double>>>(d_u2, d_u, 0);
  finalize<<<grid_dim,block_dim, blksize*size_of_double>>>(d_u1, d_u, 1);
