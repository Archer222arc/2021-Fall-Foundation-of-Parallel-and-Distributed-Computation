...
{
    double norm2 = 0;
    #pragma omp parallel for reduction(+:norm2) collapse(2) 
    for (int i = 0; i < N; i ++)
    {
        for (int j = 0; j < N; j ++)
        {
...
void jacobi(double *__restrict__ u, double *__restrict__ b, double *__restrict__ ut, int epoch, double *normr2)
{
    double normr = 0;
    if (epoch == 0){
    #pragma omp parallel for collapse(2) reduction(+:normr)
        for (int i = 0; i < N; i++)
        {   
            for (int j = 0; j < N; j++)
            {
                for (int k = 0; k < N; k++)
                {
                double tmp = b[i * NN + j * N + k];
                    ut[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1]  = tmp/6;
                    normr += tmp*tmp;
                }
            }
        }
    ...
    #pragma omp parallel for collapse(3) schedule(static,N)  reduction(+:normr)
        for (int i = 0; i < N; i++)
        {   
            for (int j = 0; j < N; j++)
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    double tmp = b[i * NN + j * N + k]
                        ...
                        + u[(i + 2) * N2N + (j + 1) * (N + 2) + k + 1];
                    ut[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1]  = tmp/6;
                    tmp = tmp - 6*u[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1];
                    normr += tmp * tmp;
                }
            }
        }
        ...
        {   
            for (int j = 0; j < N; j++)
            {
                ...
                for (int k = 0; k < N; k++)
                {
                    ...
                    u[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1]  = tmp/6;
                    tmp = tmp - 6*ut[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1];
                    normr += tmp * tmp;
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (k%2 == 1)
        final_normr = residual_norm(u, b); // Please ensure that this residual_norm is exact.
    else 
        final_normr = residual_norm(ut, b);
