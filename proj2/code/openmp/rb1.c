...
double residual_norm1(double *__restrict__ u, double *__restrict__ b)
{
    double norm2 = 0;
    double r;
#pragma omp parallel for reduction(+:norm2) private(r) collapse(2) schedule(static,N)
    for (int i = 0; i < N; i ++)
    {
        for (int j = 0; j < N; j ++)
        {
            for (int k = (i+j+1)%2; k < N; k+=2)
            {
                r = b[i * NN + j * N + k] + 
                    + u[(i + 0) * N2N + (j + 1) * (N + 2) + k + 1]
                    + u[(i + 1) * N2N + (j + 0) * (N + 2) + k + 1]
...
void rb(double *__restrict__ u, double *__restrict__ b, double *norm2)
{
    double r;
    double norm = 0;
    double tmp;
#pragma omp parallel for reduction(+:norm) collapse(2) private(tmp) 
    for (int i = 0; i < N; i ++ )
    {   
        for (int j = 0; j < N; j ++)
        {
            for (int k = (i+j+1)%2; k < N; k +=2)
            {
                tmp = b[i * NN + j * N + k] 
                    ...
                    + u[(i + 1) * N2N + (j + 2) * (N + 2) + k + 1] 
                    + u[(i + 2) * N2N + (j + 1) * (N + 2) + k + 1];
                r = tmp- 6.0*u[(i + 1) * N2N + (j + 1) * (N + 2) + k + 1];
